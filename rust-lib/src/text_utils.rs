// Maneja las diferencias carácter a carácter entre textos
pub fn get_text_changes(old: &str, new: &str) -> Vec<(usize, usize)> {
    ...
    // Manejar diferencias de longitud
    if i < old.len() {
        changes.push((j, j)); // Change represents insertion at current `j` in new
    } else if j < new.len() {
...
// Combina rangos de cambios cercanos
pub fn merge_nearby_changes(mut changes: Vec<(usize, usize)>) -> Vec<(usize, usize)> {
    ...
    let max_gap = 30; // Caracteres máximos entre cambios a combinar
...
pub fn text_difference_ratio(a: &str, b: &str) -> f64 {
    ...
    // Contar caracteres restantes como diferencias
    different += (a.len() as isize - i as isize).abs() as usize;
    different += (b.len() as isize - j as isize).abs() as usize; // count remaining chars in b as well
